...
func clearEnv(t *testing.T) {
	...
	for _, key := range []string{
		...
		"CRON_SCHEDULE", "FETCH_ON_STARTUP", "ADMIN_USERNAME", "ADMIN_PASSWORD",
...
func TestLoadDefaults(t *testing.T) {
	...
	if cfg.FetchOnStartup {
		t.Error("FetchOnStartup = true, want false by default")
	}
...
func TestLoadRespectsExplicitEnv(t *testing.T) {
	clearEnv(t)
	t.Setenv("FETCH_ON_STARTUP", "true")
	...
	if !cfg.FetchOnStartup {
		t.Error("FetchOnStartup = false, want true (explicitly set)")
	}
