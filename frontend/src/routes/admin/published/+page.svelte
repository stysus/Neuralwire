<script lang="ts">
	...
	async function fetchPublished(pageNumber: number) {
		...
		try {
			const res = await fetch(
				`${BASE_URL}/admin/news?status=published&page=${pageNumber}&page_size=${pageSize}`,
				{
					headers: {
	...
	async function handleDeleteAll() {
		...
		if (
			!confirm(
				'WARNING: Are you sure you want to delete ALL published articles? This action cannot be undone.'
			)
		) {
			return;
		}
		...
		isDeletingAll = true;
		try {
			const res = await fetch(`${BASE_URL}/admin/news?status=published`, {
				method: 'DELETE',
				headers: {
					Authorization: `Bearer ${token}`
				}
			...
			if (res.ok) {
				...
				totalItems = 0;
				totalPages = 0;
				goto('/admin/published?page=1');
			} else {
				alert('Failed to delete all published articles.');
			}
		} catch (err) {
	...
	>
		...
		<div class="flex flex-wrap items-center gap-4">
			<div class="font-mono text-xs text-slate-500">
				Total Published: <span class="font-bold text-slate-300">{totalItems}</span> Post(s)
			</div>
			<button
