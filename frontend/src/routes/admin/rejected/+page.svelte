	...
	async function fetchRejected(pageNumber: number) {
		...
		try {
			...
			if (res.ok) {
				...
				errorMessage = 'Failed to fetch rejected log index.';
			}
		} catch (err) {
		...
		try {
			...
			if (res.ok) {
				...
				if (articles.length === 0 && currentPage > 1) {
					goto(`/admin/rejected?page=${currentPage - 1}`);
				} else {
					fetchRejected(currentPage);
				}
			} else {
...
<svelte:head>
	<title>Rejected Log Index | Neuralwire</title>
</svelte:head>
...
>
	...
	>
		...
		<div class="flex flex-wrap items-center gap-4">
			...
			>
				{isDeletingAll ? 'Deleting All...' : 'Delete All Rejected'}
			</button>
		</div>
	</div>
	...
	{:else if articles.length === 0}
		...
		>
			<p class="mb-1 font-mono text-xs tracking-wider text-slate-500 uppercase">Log Empty</p>
			<p class="font-sans text-xs text-slate-600">
				No rejected articles registered in the system registry.
			</p>
		</div>
	{:else}
		<!-- List -->
